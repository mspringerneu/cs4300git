...
public class View {
    ...
    private void initObjects(GL3 gl) throws FileNotFoundException
    {
        ...
        transform = new Matrix4f().rotate((float)Math.toRadians(90),1,0,0)
                .scale(100,100,100)
    ...
    public void draw(GLAutoDrawable gla) {
        ...
        /*
        Replace lookAt(e,l, up)
        1) Find W = e - l, normalize it
        2) Let v = up
        3) Find u = V x W, normalize it
        4) Find V = W x U
        5) M world<-view = ?
        6) Compute


        Mview<-world = Mview<-v' * Mv'<-world

        Mview<-v' = | Ux Uy Uz 0 |    Mv'<-world = | 1  0  0  -Ex |
                    | Vx Vy Vz 0 |                 | 0  1  0  -Ey |
                    | Wx Wy Wz 0 |                 | 0  0  1  -Ez |
                    | 0  0  0  1 |                 | 0  0  0  1   |

        Mview<-world =  | Ux  Uy  Uz  -(U o E) |
                        | Vx  Vy  Vz  -(V o E) |
                        | Wx  Wy  Wz  -(W o E) |
                        | 0   0   0   1        |
        ...
        if (cameraMode == TypeOfCamera.GLOBAL) {
            Matrix4f cameraRotation  = new Matrix4f().rotate(1.0f * time, 0f, 1f, 0f);
            Vector4f camera = new Vector4f(0f, 400f, 600f, 1f);
            camera = camera.mul(cameraRotation);
            modelView.peek().lookAt(new Vector3f(camera.x, camera.y, camera.z), new Vector3f(0, 0, 0), new Vector3f(0, 1, 0));
        }
        else {
            //modelview.top() = modelview.top() * glm::rotate(glm::mat4(1.0),glm::radians(-90.0f),glm::vec3(1.0f,0.0f,0.0f)) * glm::inverse(animation_transform[3] * objectsList[3]->getTransform()) ;
        ...
        for (ObjectName i:ObjectName.values())
        {
            ...
            Matrix4f transform = new Matrix4f(animationTransforms.get(i))
                    .mul(meshTransforms.get(i));
    ...
    private void animate()
    {
        ...
        animationTransforms.put(ObjectName.AEROPLANE,new Matrix4f().rotate(
                (float)Math.toRadians(-50.0f*time),0,1,0).translate(200,50,0)
